...
for (const secao of listaDeSecoes) {
  cabecalhoNav.innerHTML += `<li class="nav-item" id="${secao}"> <a class="nav-link"></a> </li>`;
}
...
for (const redeSocial of listaRedesSociaisRodape) {
...
function irSecao(secao) {
  if (lingua === 'portugues') {
    corpo.innerHTML = objSecoesPT[secao];
  } else {
    corpo.innerHTML = objSecoesEN[secao];
  }
  window.scrollTo(0, 0);
  ativo(secao);
}
...
function trocaLingua() {
  if (lingua === 'portugues') {
    btnTrocaLingua.innerHTML = 'versão em português';
    document.querySelector('.desenvolvido').innerHTML = 'Developed by Mateus Souza Oliveira';
    for (const secao of listaDeSecoes) {
      document.querySelector(`#${secao}`).querySelector('a').innerHTML = objNomeSecoesEN[secao];
    }
    lingua = 'english';
  } else {
    btnTrocaLingua.innerHTML = 'english version';
    document.querySelector('.desenvolvido').innerHTML = 'Desenvolvido por Mateus Souza Oliveira';
    for (const secao of listaDeSecoes) {
      document.querySelector(`#${secao}`).querySelector('a').innerHTML = objNomeSecoesPT[secao];
    }
    lingua = 'portugues';
  }
